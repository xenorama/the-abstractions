...
var kslider = { height: 0, width: 0, h_pos: 0, v_pos: 0, margin: 0, keys: { white : { width: 0, indices: [0,2,4,5,7,9,11] }, black: { width: 0, indices: [1,3,0,6,8,10] } }, line_width: 1.5 }
...
function dictionary(u) {
  if (inlet == 0) {
    ...
    if (dict.contains("root") && dict.get("root") !== "*") tones.root = dict.get("root")
    ...
    scales = new Dict(u)
    post(user_scales.getkeys())
  }
}

function get_chord(){
  tones.lowest.string = -1
  for (c=0;c<model.num_strings;c++){
    ...
    if (tones.chord.wrap[c] !== -1) {
      ...
      if (tones.lowest.string > c || tones.lowest.string == -1) tones.lowest = {
        string: c,
...
function paint(){
    ...
    for (f = 0; f <= model.frets; f++) {
      ...
      if (draw_fret){
        for (s = 5; s >= 0; s--){
          ...
          var tone_check = (tone+11-tones.root) % 12
  ...
  translate(model.begin,0)
    for (s = 0; s < model.num_strings; s++){
      ...
      var tone = (offset+tones.strings.open[5-s]) % 12
      var tone_check = (tone+11-tones.root) % 12
      if (tones.scale.keys.indexOf((tones.strings.open[5-s]+12-tones.root) % 12) == -1 && strings.peek(1,5-s) == -1) {
    ...
    for (k=0;k<7;k++) {
      ...
      if (scale_redux.indexOf((kslider.keys.white.indices[k]+12-tones.lowest.pitch) % 12) !== -1) {
        if (tones.root == kslider.keys.white.indices[k]) set_source_rgba(color.string)
    ...
    for (k=0;k<6;k++) {
      ...
      if (scale_redux.indexOf((kslider.keys.black.indices[k]+12-tones.lowest.pitch) % 12) !== -1) {
        if (tones.root == kslider.keys.black.indices[k]) set_source_rgba(color.string)
        else set_source_rgba(color.open)
        fill_preserve_with_alpha(0.7)
      }
      set_source_rgba(color.fret)
    ...
    identity_matrix()
    translate(kslider.h_pos,kslider.v_pos)
    move_to(0,kslider.height+font.size)
...
function change_base(string){
  ...
  for (d=0;d<10;d++){
    ...
    if (index > -1) {
      ...
      if (string > 0) {
        for (t=0;t<string;t++) strings.poke(1,t,-1)
      }
      break
    }
  }
