...
// Shadows
vec4 btex2D(sampler2DRect map, vec2 uv, float radius, float steps) {
...
float compute_shadow(sampler2DRect shadowMap, vec4 shadowMapPos, float ourDepth, vec3 param)	{
	// param.x = radius
	// param.y = epsilon
	// param.z = min clamp
