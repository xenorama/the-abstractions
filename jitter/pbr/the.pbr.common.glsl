...
void getTriplanarUVs(inout geometry geom){
	geom.uvX = jit_in.modelPos.zy; //changed the swizzling from yz to zy
...
void binarySearch(inout vec3 P, inout float H, in vec3 offset){
	...
	for(int j = 0; j < 10; j++){
		//prevP = P;
		//prevH = H;
		H = (1. - getHeight(P.xy))*abs(heightScale);
...
float selfShadow(vec3 L, in geometry geom){
	vec3 	P = vec3(geom.uv, geom.height);
	...
	for(i = 0; i < shadowIterations; i++){
		...
		float 	H = (1. - getHeight(P.xy))*abs(heightScale);
...
float getAO(in geometry geom, float currH){
	...
	float 	occ = 0.;
	vec2 	off = vec2(occlusionOffset, 0.);
